...
tow.13.kgC.pk <- cbind(tow.13.kgC[,c("plot","spp")],
	apply(tow.13.kgC[, 5:ncol(tow.13.kgC)], 1, max,na.rm=T))
tow.ann.sum.spp <- rbind(tow.ann.sum.spp,
	c(2013,tapply(tow.13.kgC.pk[, 3], tow.13.kgC.pk[,"spp"], sum)/
	1000/(10^2 * pi * 34) * 10000))	
	
	tow.14.kgC.pk <- cbind(tow.14.kgC[,c("plot","spp")],
	apply(tow.14.kgC[, 5:ncol(tow.14.kgC)], 1, max,na.rm=T))
	#all stripes maple died in 2014 so empty column needs to be added back in
	tow.14.kgC.pk<-rbind(tow.14.kgC.pk,c(NA,"sm",0))
	tow.14.kgC.pk[,3]<-as.numeric(tow.14.kgC.pk[,3])
tow.ann.sum.spp <- rbind(tow.ann.sum.spp,
	c(2014,tapply(tow.14.kgC.pk[, 3], tow.14.kgC.pk[,"spp"], sum)/
	1000/(10^2 * pi * 34) * 10000))
...
par(mar=c(5,5.5,1,1),las=1)
matplot(as.numeric(names(agwi.spp.dom[1:17,1])), agwi.spp.dom[1:17,],
...
axis(side=1,at=1998:2014,labels=F)
