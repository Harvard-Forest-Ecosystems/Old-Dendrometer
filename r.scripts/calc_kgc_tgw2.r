"calc_kgc" = function(spp, dbh)
{
  ...
  spp <- tolower(spp)
	kgc <- dbh
	known.spp <- c("ash", "bb", "beech", "bo", "cherry", "gb", "hem", "rm", "ro", "rp", "chestnut", "sm",
	  "wb", "wh", "wo", "wp", "ws", "yb", "hbb", "nwr", "spi", "fpc", "haw", "ss")
...
#Betula alleghaniensis (Yellow birch) 
	yb <- which(spp == "yb")
	if(sum(yb) > 0)
		kgc[yb] <- (10^(2.1413 + 2.2683 * log10(dbh[yb])) + 10^(1.0535 + 2.7995 * log10(dbh[yb])))/1000
	#----------------------------------------------
...
#Vaccinium spp. (high bush blueberry) & Rhus typhina (Staghorn sumac)
	...
	if (sum(hbb) > 0)
		kgc[hbb] <- (95.143 * (dbh[hbb])^3.706)/1000
...
#Prunus pennsylvanica (pin cherry)
  ...
  # for hawthorn spp use the general equation for mixed hardwood from Jenkins et al.
	...
	haw <- which(spp == "haw" | !spp %in% known.spp)
	if (sum(haw) > 0)
